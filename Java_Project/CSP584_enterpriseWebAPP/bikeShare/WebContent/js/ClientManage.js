...
function to_page(path, page) {
    $.ajax({
        url: path + "/AdminUserServlet?flag=showjson",
        data: "page=" + page,
        type: "get",
...
function build_user_table(path,result) {
    ...
    if (result.data.length > 0) {
        ...
        var userid = $("<td></td>").append(result.data[i].adminid);
        var username = $("<td></td>").append(result.data[i].adminname);
        var password = $("<td></td>").append(result.data[i].password);
        var dept = $("<td></td>").append(result.data[i].dept);
