...
public class MySqlDataStoreUtilities {
	...
	public static void insertOrder(int orderId, String userName, String orderName, double orderPrice,
			String userAddress, String creditCardNo, String placedDate, String deleveryDate) {
		try {

			getConnection();
			String insertIntoCustomerOrderQuery = "INSERT INTO customerOrders(OrderId,UserName,OrderName,OrderPrice,userAddress,creditCardNo,orderDiscount, deleveryDate) "
					+ "VALUES (?,?,?,?,?,?,?,?);";
			...
			pst.setString(7, placedDate);
			pst.setString(8, deleveryDate);
			pst.execute();
		} catch (Exception e) {
		...
		try {
			...
			while (rs.next()) {
				...
				OrderPayment order = new OrderPayment(rs.getInt("OrderId"), rs.getString("userName"),
						...
						rs.getString("creditCardNo"), rs.getString("placedDate"), rs.getString("deleveryDate"));
...
//			System.out.println(acc.getId() +"$"+ acc.getName()  +"$"+ acc.getPrice()+"$"+ acc.getImage() +"$"+ acc.getLabel() +"$"+ acc.getCondition()+"$"+acc.getDiscount());

			}

			for (Map.Entry<String, Wearable_Technology> entry : SaxParserDataStore.wearable_Technologies.entrySet()) {
				Wearable_Technology con = entry.getValue();
				String name = "wearableTechnologies";
...
//				System.out.println(con.getId() +"%%%%%%%%%%%"+ accentry.getValue());
			...
			for (Map.Entry<String, Laptop> entry : SaxParserDataStore.laptops.entrySet()) {
				...
				pst.setString(6, tablet.getLabel());
	...
	public static HashMap<String, Wearable_Technology> getConsoles() {
		...
		try {
			...
			pst.setString(1, "wearableTechnologies");
			ResultSet rs = pst.executeQuery();

			while (rs.next()) {
				Wearable_Technology con = new Wearable_Technology(rs.getString("productName"),
						rs.getDouble("productPrice"), rs.getString("productImage"), rs.getString("productManufacturer"),
	...
	public static HashMap<String, Phone> getGames() {
	...
	public static HashMap<String, Laptop> getTablets() {
		...
		try {
			...
			while (rs.next()) {
				Laptop tab = new Laptop(rs.getString("productName"), rs.getDouble("productPrice"),
						rs.getString("productImage"), rs.getString("productManufacturer"),
						rs.getString("productCondition"), rs.getDouble("productDiscount"));
