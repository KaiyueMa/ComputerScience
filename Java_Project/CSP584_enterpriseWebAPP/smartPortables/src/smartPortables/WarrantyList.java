...
@WebServlet("/WarrantyList")

public class WarrantyList extends HttpServlet {
...
//		Console console = hm.get(ConsoleName);
		...
		for(Map.Entry<String, Wearable_Technology> entry : hm.entrySet())
		{
			...
			for(Map.Entry<String, String> acc:wearable_Technology.getAccessories().entrySet())
			{
		        
				Warranty warranty= SaxParserDataStore.warranties.get(acc.getValue());
				if(i%2==1) pw.print("<tr>");
//				System.out.print(size);
				pw.print("<td><div id='shop_item'>");
				pw.print("<h3>"+warranty.getName()+"</h3>");
				pw.print("<strong>"+warranty.getPrice()+"$</strong><ul>");
				pw.print("<li id='item'><img src='images/accessories/"+warranty.getImage()+"' alt='' /></li>");
				pw.print("<li><form method='post' action='Cart'>" +
						"<input type='hidden' name='name' value='"+acc.getValue()+"'>"+
						"<input type='hidden' name='type' value='warranties'>"+
						"<input type='hidden' name='maker' value='"+CategoryName+"'>"+
						"<input type='hidden' name='access' value='"+wearable_Technology.getName()+"'>"+
