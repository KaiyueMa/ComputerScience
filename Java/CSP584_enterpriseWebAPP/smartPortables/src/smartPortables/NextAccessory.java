...
public class NextAccessory extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else
		{
		   if(CategoryName.equals("microsoft"))
		   {
			...
			for(Map.Entry<String,Wearable_Technology> entry : SaxParserDataStore.wearable_Technologies.entrySet())
				{
				 if(entry.getValue().getLabel().equals("Sony"))
				 {
					 hm.put(entry.getValue().getId(),entry.getValue());
				 }
				}
			...
			else if(CategoryName.equals("nintendo"))
			{
				for(Map.Entry<String,Wearable_Technology> entry : SaxParserDataStore.wearable_Technologies.entrySet())
				{
				 if(entry.getValue().getLabel().equals("Nintendo"))
				 {
					 hm.put(entry.getValue().getId(),entry.getValue());
				 }
				}
	...
	//	int i = 1; int size= hm.size();
		...
		pw.print("</h2><div class='entry'><table id='bestseller'>");
		Wearable_Technology wearable_Technology1 = hm.get(ProductName);
//		System.out.print(ProductName);
		int i = 1; int size= hm.size();
		pw.print("<tr>");
		ArrayList<String> arr = new ArrayList<String> ();
		for(Map.Entry<String, String> acc:wearable_Technology1.getAccessories().entrySet())
		{
