...
public class ProductRecommenderUtility{
	...
	public HashMap<String,String> readOutputFile(){
		...
		try {
            ...
            br = new BufferedReader(new FileReader(new File("/Users/michael/Codes/Java-eclipse/smartPortables/WebContent/matrixFactorizationBasedRecommendations.csv")));
	...
	public static Product getProduct(String product){
		...
		try 
		{
			...
			while(rs.next())
			{	
				prodObj = new Product(rs.getString("Id"),rs.getString("productName"),
						rs.getDouble("productPrice"),rs.getString("productImage"),
						rs.getString("productManufacturer"),rs.getString("productCondition"),
						rs.getString("ProductType"),rs.getDouble("productDiscount"));
			}
			rs.close();
			pst.close();
			conn.close();
