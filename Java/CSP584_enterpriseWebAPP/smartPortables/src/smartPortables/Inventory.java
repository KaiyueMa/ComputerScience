...
public class Inventory extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		pw.print("<h4 style='color:black'>Manufacturer Rebates Table : </h4>");
		pw.print("<form name ='ManufacturerRebatesProducts' action='BarChart' method='post'>");
			pw.print("<table  class='gridtable'>");
			int k = 1;

			pw.print("<td>Product Name</td><td>Price</td><td>Available Numbers</td><td>Manufacturer Rebates</td>");
			for(Map.Entry<String, Wearable_Technology> entry : hm.entrySet()){
				Wearable_Technology console = entry.getValue();
				
				pw.print("<tr>");
				pw.print("<td>"+console.getName()+"</td><td>: "+console.getPrice()+"</td></td><td>"+ console.getQuantiltiesOfAvailable() +"</td><td>" +console.getDiscount()+ "</td>" );
				pw.print("<input type='hidden' name='orderName' value='"+console.getName()+"'>");
				pw.print("<input type='hidden' name='orderIndex' value='"+ k +"'>");
				pw.print("<input type='hidden' name='orderPrice' value='"+console.getQuantiltiesOfAvailable()+"'>");
				pw.print("<input type='hidden' name='manufacturerRebates' value='"+console.getDiscount()+"'>");
				pw.print("</tr>");	
				k++;
			}
			...
			pw.print("<h4 style='color:black'>Onsale Tables : </h4>");
			pw.print("<form name ='OnsaleProducts' action='BarChart' method='post'>");
				pw.print("<table  class='gridtable'>");
				int j = 1;

				pw.print("<td>Product Name</td><td>Price</td><td>Available Numbers</td><td>Discount</td>");
				for(Map.Entry<String, Wearable_Technology> entry : hm.entrySet()){
					Wearable_Technology console = entry.getValue();
				...
				pw.print("</table></form>");
					...
					pw.print("<h4 style='color:black'>Available Number Table : </h4>");
					pw.print("<form name ='AvailableProducts' action='BarChart' method='post'>");
						pw.print("<table  class='gridtable'>");
						int i = 1;

						pw.print("<td>Product Name</td><td>Price</td><td>Available Numbers</td>");
						for(Map.Entry<String, Wearable_Technology> entry : hm.entrySet()){
							Wearable_Technology console = entry.getValue();
							
							pw.print("<tr>");
							pw.print("<td>"+console.getName()+"</td><td>: "+console.getPrice()+"</td></td><td>"+ console.getQuantiltiesOfAvailable() +"</td>" );
							pw.print("<input type='hidden' name='productName' value='"+console.getName()+"'>");
							pw.print("<input type='hidden' name='orderIndex' value='"+ i +"'>");
							pw.print("<input type='hidden' name='quantiltiesOfAvailable' value='"+console.getQuantiltiesOfAvailable()+"'>");
							pw.print("<input type='hidden' name='formName' value='AvailableProducts'>");
							pw.print("</tr>");	
							i++;
						}
			        ...
			        pw.print("<h2 class='title meta'><a style='font-size: 24px;'>Avaliable Number BarChart Analysis : </a></h2>"
			                + "<div class='entry'>");
			        ...
			        for(Map.Entry<String, Wearable_Technology> entry : hm.entrySet()){
						...
						pw.println("        data.addRows([\r\n" + 
				        		"          ['"+ console.getName()+"',"+ console.getQuantiltiesOfAvailable() + "],\r\n" + 
				        		"        ]);");
			        }		
			        
			        
			        pw.println("        // Set chart options\r\n" + 
			        		"        var options = {'title':'Avaliable Numbers',\r\n" + 
			        		"                       'width':700,\r\n" + 
			        		"                       'height':600};\r\n" + 
			        		"\r\n" + 
