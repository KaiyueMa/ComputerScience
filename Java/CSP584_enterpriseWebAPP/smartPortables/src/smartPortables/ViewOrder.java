			...
			{	
				...
				if (orderPayments.get(orderId) != null) {
					for (OrderPayment od : orderPayments.get(orderId)) {
						if (username.equals("salesman") || od.getUserName().equals(username)) {
				...
				{	
					...
					for (OrderPayment oi : orderPayments.get(orderId)) 
					{
						...
						pw.print("<td>"+oi.getOrderId()+"</td><td>"+oi.getUserName()+"</td><td>"+oi.getOrderName()+"</td><td>Price: "+oi.getOrderPrice()+"</td><td>Delivered Date: "+oi.getDeliveredDate()+"</td>");
//						pw.print("<td><input type='submit' name='Order' value='CancelOrder' class='btnbuy'></td>");
		...
		if(request.getParameter("Order")!=null && request.getParameter("Order").equals("CancelOrder"))
		{
			...
			java.text.SimpleDateFormat format = new java.text.SimpleDateFormat("E yyyy.MM.dd ");
...
//							if(oi.getOrderName().equals(orderName) && oi.getUserName().equals(username))
							...
							{
								try {
									deliveredDate = format.parse(oi.getDeliveredDate());
								} catch (ParseException e) {
									e.printStackTrace();
								} catch (java.text.ParseException e) {
									e.printStackTrace();
								}
								long day = (deliveredDate.getTime()-today.getTime())/(24*60*60*1000);
								
								if(day < 6) {
									pw.print("<h4 style='color:red'>Sorry! Can not cancel the order. </h4>");
